...
const colorMatch = [1 - 0.2126,1 - 0.7153,1 - 0.0721,0  ]

function loadBm(imgCanvas,colorMatch) {
	var ctx = imgCanvas.getContext('2d');
	var bm = new Bitmap(imgCanvas.width, imgCanvas.height);
	var imgdataobj = ctx.getImageData(0, 0, bm.w, bm.h);
	let data = imgdataobj.data;
	var l = data.length, i, j, color;
	let r = colorMatch[0], g = colorMatch[1] , b = colorMatch[2], a = colorMatch[3];
	for (i = 0, j = 0; i < l; i += 4, j++) {

		color = data[i] * r + data[i + 1] * g + data[i + 2] * b + data[i + 3] * a
...
function getSvgPathString(pathlist, size) {
	function path(curve) {
...
function getSVG(pathString, w, h, opt_type) {
	...
	return `<svg id="svg" version="1.1" width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg">
		<path d="${pathString}" stroke="${strokec}" fill="${fillc}" ${fillrule} /></svg>`
}
...
export function tracePathList (imgElement, options) {
	let info = getInfo(options)
	let imgCanvas = loadCanvas(imgElement);
	let bm = loadBm(imgCanvas,options.colorMatch);
	return bmToPathlist(bm,info);
}

export function tracePath (imgElement, options) {
	let info = getInfo(options)
	let imgCanvas = loadCanvas(imgElement);
	let bm = loadBm(imgCanvas,options.colorMatch);
	let pathlist = bmToPathlist(bm,info);
	processPath(pathlist,info);
	return getSvgPathString(pathlist, info.svg_size)
}


export function traceContour (imgElement, options) {
	let info = getInfo(options)
	let imgCanvas = loadCanvas(imgElement);
	let bm = loadBm(imgCanvas,options.colorMatch);
	let pathlist = bmToPathlist(bm,info).filter(path => path.sign === "+")
	processPath(pathlist,info);
	return getSvgPathString(pathlist, info.svg_size)
}

export function traceSvg (imgElement, options ) {
	let info = getInfo(options)
	let imgCanvas = loadCanvas(imgElement);
	let bm = loadBm(imgCanvas,options.colorMatch);
	let pathlist = bmToPathlist(bm,info);
	processPath(pathlist,info);
	let pathString = getSvgPathString(pathlist,info.svg_size)
	return getSVG(pathString,bm.w * info.svg_size, bm.h * info.svg_size,  info.svg_opt_type )
}
