...
import { consolidateSpaces, getTextRenderingMode, removeNewlines, replaceTabsBySpace, transformText, transformXmlSpace, trimLeft, trimRight } from '../utils/text';
...
export class TextNode extends GraphicsNode {
    async renderCore(context) {
        ...
        if (tSpanCount === 0) {
            ...
            let currentTextSegment = new TextChunk(this, context.attributeState.textAnchor, textX + dx, textY + dy);
            for (let i = 0; i < this.element.childNodes.length; i++) {
                const textNode = this.element.childNodes[i];
