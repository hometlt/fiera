import { getAttribute, nodeIs } from './utils/node.js';
import { toPixels } from './utils/misc.js';
import { parseColor, parseFloats } from './utils/parsing.js';
import FontFamily from './imports/font-family.js';
import { findFirstAvailableFontFamily, fontAliases } from './utils/fonts.js';
import { parseFill } from './fill/parseFill.js';
import { ColorFill } from './fill/ColorFill.js';
import { GState } from './../../jspdf/src/jspdf.js';
export function parseAttributes(context, svgNode, node) {
    ...
    // stroke mode
    let strokeWidth = getAttribute(domNode, context.styleSheets, 'stroke-width');
    ...
    let dashArray = getAttribute(domNode, context.styleSheets, 'stroke-dasharray');
    if (dashArray) {
        dashArray = parseFloats(dashArray);
        const dashOffset = parseInt(getAttribute(domNode, context.styleSheets, 'stroke-dashoffset') || '0');
    ...
    const alignmentBaseline = getAttribute(domNode, context.styleSheets, 'vertical-align') ||
        getAttribute(domNode, context.styleSheets, 'alignment-baseline');
    if (alignmentBaseline) {
        const matchArr = alignmentBaseline.match(/(baseline|text-bottom|alphabetic|ideographic|middle|central|mathematical|text-top|bottom|center|top|hanging)/);
        if (matchArr) {
...
export function applyAttributes(childContext, parentContext, node) {
    let fillOpacity = 1.0, strokeOpacity = 1.0;
