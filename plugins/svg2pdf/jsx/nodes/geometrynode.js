import { Marker, MarkerList } from '../markerlist.js';
import { Close, CurveTo, LineTo, MoveTo } from '../utils/path.js';
import { iriReference } from '../utils/constants.js';
import { addVectors, getAngle, getDirectionVector, normalize } from '../utils/geometry.js';
import { getAttribute } from '../utils/node.js';
import { GraphicsNode } from './graphicsnode.js';
export class GeometryNode extends GraphicsNode {
    constructor(hasMarkers, element, children) {
    ...
    async fillOrStroke(context) {
        ...
        const isNodeFillRuleEvenOdd = getAttribute(this.element, context.styleSheets, 'fill-rule') === 'evenodd';
    ...
    getMarkers(path, context) {
        let markerStart = getAttribute(this.element, context.styleSheets, 'marker-start');
        let markerMid = getAttribute(this.element, context.styleSheets, 'marker-mid');
        let markerEnd = getAttribute(this.element, context.styleSheets, 'marker-end');
        ...
        if (markerStart || markerMid || markerEnd) {
            ...
            let prevAngle = [1, 0], curAngle, first = false, firstAngle = [1, 0], last = false;
            for (let i = 0; i < list.length; i++) {
                const curr = list[i];
                const hasStartMarker = markerStart &&
                ...
                if (prev instanceof MoveTo || prev instanceof LineTo || prev instanceof CurveTo) {
                    if (curr instanceof CurveTo) {
                        ...
                        if (hasMidMarker) {
                            const angle = curr instanceof MoveTo
                                ? prevAngle
                        ...
                        if (hasMidMarker) {
                            const angle = prev instanceof MoveTo ? curAngle : normalize(addVectors(prevAngle, curAngle));
                            markers.addMarker(new Marker(markerMid, [prev.x, prev.y], Math.atan2(angle[1], angle[0])));
                        }
                        if (hasEndMarker) {
