...
export function parseXml(xml,warningCallback) {
...
export function SVGToPDFKit(doc, svg, x, y, options) {
    "use strict";
    ...
    async function createSVGElement(obj, inherits) {
      switch (obj.nodeName) {
        ...
        case 'image': return await new SVGElemImage(obj, inherits).promise()
    ...
    class SvgElem {
      constructor(obj, inherits){
        this.__promise = new Promise(async (resolve) => {
          this.__styleCache = Object.create(null);
          this.childrenCache = null;
          this.name = obj.nodeName;
          this.isOuterElement = obj === svg || !obj.parentNode;
          this.inherits = inherits || (!this.isOuterElement ? await createSVGElement(obj.parentNode, null) : null);
      ...
      computeUnits(value, unit, percent, isFontSize) {
        if (unit === '%') {
      ...
      computeLength(value, percent, initial, isFontSize) {
      ...
      computeLengthList(value, percent, strict) {
      ...
      getLength (key, percent, initial) {
        return this.computeLength(this.attr(key), percent, initial);
      };
      getLengthList(key, percent) {
      ...
      get(key) {
        if (this.__styleCache[key] !== undefined) {return this.__styleCache[key];}
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (value === 'inherit') {
            ...
            if (result != null) {return this.__styleCache[key] = result;}
          }
          ...
          if (value != null) {
            ...
            switch (key) {
              ...
              case 'stroke-dashoffset':
                ...
                break;
              default:
                return this.__styleCache[key] = value;
            }
            if (result != null) {return this.__styleCache[key] = result;}
          }
        }
        return this.__styleCache[key] = (keyInfo.inherit && this.inherits ? this.inherits.get(key) : keyInfo.initial);
      };
      getChildren() {
    ...
    class SvgElemHasChildren extends SvgElemStylable{
      ...
      allowedChildren = ['use', 'g', 'a', 'svg', 'image', 'rect', 'circle', 'ellipse', 'line', 'polyline', 'polygon', 'path', 'text'];
      getBoundingShape () {
    ...
    class SvgElemUse extends SvgElemContainer{
      constructor(obj, inherits) {
        super(obj, inherits);
        this.__x = this.getLength('x', this.getVWidth(), 0);
        this.__y = this.getLength('y', this.getVHeight(), 0);
        this.__child = this.getUrl('href') || this.getUrl('xlink:href');
        if (this.__child) {
          this.__child = createSVGElement(child, this);
        }
      }
      getChildren () {
        return this.__child ? [this.__child] : [];
      };
      drawInDocument (isClip, isMask) {
        doc.save();
        this.drawContent(isClip, isMask);
        doc.restore();
      };
      getTransformation () {
        return multiplyMatrix(this.get('transform'), [1, 0, 0, 1, this.__x, this.__y]);
      };
    }
    ...
    class SvgElemSymbol extends SvgElemContainer{
      constructor (obj, inherits) {
        super( obj, inherits);
        this.__width = this.getLength('width', this.getParentVWidth(), this.getParentVWidth())
        this.__height = this.getLength('height', this.getParentVHeight(), this.getParentVHeight())
        if (inherits instanceof SvgElemUse) {
          this.__width = inherits.getLength('width', inherits.getParentVWidth(), this.__width);
          this.__height = inherits.getLength('height', inherits.getParentVHeight(), this.__height);
        }
        this.__aspectRatio = (this.attr('preserveAspectRatio') || '').trim()
        this.__viewBox = this.getViewbox('viewBox', [0, 0, this.__width, this.__height]);
      };
      getVWidth = function() {
        return this.__viewBox[2];
      };
      getVHeight = function() {
        return this.__viewBox[3];
      };
      drawInDocument = function(isClip, isMask) {
        doc.save();
        this.drawContent(isClip, isMask);
        doc.restore();
      };
      getTransformation = function() {
        return multiplyMatrix(parseAspectRatio(aspectRatio, this.__width, this.__height, this.__viewBox[2], this.__viewBox[3]), [1, 0, 0, 1, -this.__viewBox[0], -this.__viewBox[1]]);
      };
    }
    ...
    class SVGElemImage extends SvgElemStylable{
      constructor(obj, inherits) {
        super( obj, inherits);
        this.__promise.then(async (resolve ) => {
          let link = await imageCallback(this.attr('href') || this.attr('xlink:href') || '');
              this.__x = this.getLength('x', this.getVWidth(), 0);
              this.__y = this.getLength('y', this.getVHeight(), 0);
              this.__width = this.getLength('width', this.getVWidth(), 'auto')
              this.__height = this.getLength('height', this.getVHeight(), 'auto')
          try {
            this.__image = doc.openImage(link);
          } catch (e) {
            warningCallback('SVGElemImage: failed to open image "' + link + '" in PDFKit');
          }
          if (this.__image) {
            if (this.__width === 'auto' && this.__height !== 'auto') {
              this.__width = this.__height * this.__image.width / this.__image.height;
            } else if (this.__height === 'auto' && this.__width !== 'auto') {
              this.__height = this.__width * this.__image.height / this.__image.width;
            } else if (this.__width === 'auto' && this.__height === 'auto') {
              this.__width = this.__image.width;
              this.__height = this.__image.height;
            }
          }
          if (this.__width === 'auto' || this.__width < 0) {
            this.__width = 0;
          }
          if (this.__height === 'auto' || this.__height < 0) {
            this.__height = 0;
          }
          resolve(this)
        })
      ...
      getBoundingShape = function() {
        return new SvgShape().M(x, y).L(x + this.__width, y).M(x + this.__width, y + this.__height).L(x, y + this.__height);
      };
      drawInDocument = function(isClip, isMask) {
        if (this.get('visibility') === 'hidden' || !this.__image) {return;}
        doc.save();
        this.transform();
        if (this.get('overflow') === 'hidden') {
          doc.rect(x, y, this.__width, this.__height).clip();
        }
        this.clip();
        this.mask();
        doc.translate(x, y);
        doc.transform.apply(doc, parseAspectRatio(this.attr('preserveAspectRatio'), this.__width, this.__height, this.__image ? this.__image.width : this.__width, this.__image ? ithis.__mage.height : this.__height));
        if (!isClip) {
          doc.fillOpacity(this.get('opacity'));
          doc.image(this.__image, 0, 0);
        } else {
          doc.rect(0, 0, this.__image.width, this.__image.height);
  ...
  var SvgElemTextContainer = function(obj, inherits) {
    ...
    this.drawTextInDocument = function(isClip, isMask) {
      if (this.link && !isClip && !isMask) {this.addLink();}
      if (this.get('text-decoration').includes('underline')) {
        this.decorate(0.05 * this._font.size, -0.075 * this._font.size, isClip, isMask);
      }
      if (this.get('text-decoration').includes('overline')) {
      ...
      if (this.get('text-decoration').includes('line-through')) {
    ...
    (function (textParentElem) {
      ...
      function recursive(currentElem, parentElem) {
        ...
        let fontOptions = {fauxItalic: false, fauxBold: false},
            ...
            fontNameorLink = fontCallback.call(doc,currentElem.get('font-family'), currentElem.get('font-weight'), currentElem.get('font-style'), fontOptions);
        try {
        ...
        if (matrix = validateMatrix(matrix)) {
          ...
          for (let n = 0; n < nTotal; n++) {
            ...
            for (let i = 0; i < children.length; i++) {
              ...
              if (i === 0 && offset > 0) {
                let stopOffset = n / nTotal;
                if(stopOffset === 1) _lastStop = stopColor;
                grad.stop(stopOffset, stopColor[0], stopColor[1]);
              }

              let stopOffset = (n + offset) / (nAfter + nBefore + 1);

              if(stopOffset === 1) _lastStop = stopColor;
              grad.stop(stopOffset, stopColor[0], stopColor[1]);

              if (i === children.length - 1 && offset < 1) {
                let stopOffset = (n + 1) / nTotal;
                if(stopOffset === 1) _lastStop = stopColor;
                grad.stop(stopOffset, stopColor[0], stopColor[1]);
              }
            }
          }
          //todo правка линейных градиентов с 3 и боллееточек стопа где последняя на 100%
          if(grad.stops.length > 2 && _lastStop){
            // let lastStop = grad.stops[grad.stops.length - 1];
            grad.stop(1, _lastStop[0], _lastStop[1]);
          }
