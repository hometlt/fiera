...
import Deco from "./object.deco.js";
  ...
  prototypes: {
    Editor:{
      ...
      setFrames: function (frames) {
        ...
        for(let key in frames){
          ...
          let type = fabric.util.string.camelize(frame.id);

          output[type] = Object.assign({
            frame: false,
            shape: false,
            type: "photo",
            role: "frame"
      ...
      getFrame: function (value) {
        let camelizeValue = fabric.util.string.camelize(value);
        return this.frames[camelizeValue]
        ...
        if(el.availableFrames){
          ...
          for(let frameId of el.availableFrames){
            ...
            framesList.push({
              ...
              id: frameId,
              data: fabric.util.object.clone(frame)
            })
          }
        }else {
    ...
    Object:{
      ...
      setFrame: function (value, callback) {
        if (this.canvas) {
          this.saveStates(["frame"]);
          this.canvas.stateful = false;
        }
        let shapeReady, decoReady;

        if (value) {
          this.frame = value.constructor === String ? this.editor.getFrame(value): value
