    ...
    Object: {
      ...
      getCornerData(cornerStyle, cornerName,cs){
        ...
        switch (cornerStyle) {
          ...
          case "arc":
            switch (cornerName){
              case "mtr": return {shape: {type: "circle" }}
              case "bl":  return {angle: 0,   shape: {type: "arcCorner"}}
              case "tl":  return {angle: 90,  shape: {type: "arcCorner"}}
              case "tr":  return {angle: 180, shape: {type: "arcCorner"}}
              case "br":  return {angle: 270, shape: {type: "arcCorner"}}
              case "mb":  return {angle: 0,   shape: {type: "arcSide"}}
              case "ml":  return {angle: 90,  shape: {type: "arcSide"}}
              case "mt":  return {angle: 180, shape: {type: "arcSide"}}
              case "mr":  return {angle: 270, shape: {type: "arcSide"}}
              default:    return {shape: {type: "circle" }}
            }
          case "frame" :
            switch (cornerName){
              case "mtr": return {shape: {type: "rect" }}
              case "tl":  return {angle: 0,   shape: {type: "bracketCorner"}}
              case "tr":  return {angle: 90,  shape: {type: "bracketCorner"}}
              case "br":  return {angle: 180, shape: {type: "bracketCorner"}}
              case "bl":  return {angle: 270, shape: {type: "bracketCorner"}}
              case "mt":  return {angle: 0,   shape: {type: "bracketSide"}}
              case "mr":  return {angle: 90,  shape: {type: "bracketSide"}}
              case "mb":  return {angle: 180, shape: {type: "bracketSide"}}
              case "ml":  return {angle: 270, shape: {type: "bracketSide"}}
              default:    return {shape: {type: "rect" }}
            }
          case "canva":
            let wh = this._calculateCurrentDimensions()
            switch (cornerName){
              case "mtr": return{shape: {type: "circle", size: cs * 1.2},label: {text: "\uf021", size: cs * 0.75}}
              case "mr":  return{shape: {type: "roundRect", width: cs * 0.4, height: Math.max(cs * 0.4, Math.min(cs * 6, wh.y - cs * 2)), radius: cs * 0.2 }}
              case "ml":  return{shape: {type: "roundRect", width: cs * 0.4, height: Math.max(cs * 0.4, Math.min(cs * 6, wh.y - cs * 2)), radius: cs * 0.2 }}
              case "mt":  return{shape: {type: "roundRect", width: Math.max(cs * 0.4, Math.min(cs * 6, wh.x - cs * 2)), height: cs * 0.4, radius: cs * 0.2 }}
              case "mb":  return{shape: {type: "roundRect", width: Math.max(cs * 0.4, Math.min(cs * 6, wh.x - cs * 2)), height: cs * 0.4, radius: cs * 0.2 }}
              default:    return {shape: {type: "circle" }}
            }
        }
      },
