...
function beautyStringify(value, offset = 0, useDoubleQuotes = false ) {
  ...
  let finalPrefix = Array(offset + 1).join("  ")
  let prefix = finalPrefix + "  ";

  if (Array.isArray(value)) {
    if (value.length) {
      let props = [];
      if(value[0] && (value[0].constructor === Object || value[0].constructor === Array)){
        value.forEach(el => {
          props.push(`${prefix}${beautyStringify(el, offset + 1, useDoubleQuotes)}`)
        })
        return `[\n${props.join(",\n")}\n${finalPrefix}]`;
      }else{
        value.forEach(el => {
          props.push(`${beautyStringify(el, 0, useDoubleQuotes)}`)
        })
        return `[${props.join(",")}]`;
      }
    }
    ...
    if (keys.length) {
      function getKey(key) {
        return !useDoubleQuotes && beautyStringifyValid.test(key) ? key : '"' + key + '"';
      }

      let props = keys.map(key => `${prefix}${getKey(key)}: ${beautyStringify(value[key], offset + 1, useDoubleQuotes)}`)
        .join(",\n")
      return `{\n${props}\n${finalPrefix}}`;
    }
    return JSON.stringify(value)
...
const bannerString =`Fiera.js. Set of tool for FabricJS library.\n@author Denis Ponomarev <ponomarevtlt@gmail.com>\n@date ${dateString}`;
module.exports.bannerString = bannerString;


//get list of installed NPM modules
export function npmls(globally) {
...
export function copyFiles(config){
  ...
  let destinationArray = require("./dest.json")
...
export function collectInfo(config){
  ...
  let optimizationMinimizer;
  if(config.optimization && config.optimization.minimizer){
    optimizationMinimizer = config.optimization.minimizer;
