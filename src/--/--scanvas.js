...
Object.assign(fabric.StaticCanvas.prototype, CanvasMixin,{
	...
	load: function(options,callback){
		...
		this.editor && this.editor.fire("slide:loading:before", {target: this, type: "slide", options: options});

		// console.log("loading:before",options.id);
		this.set(options, () => {
			this.loaded = true;
			...
			this.renderAll();
			setTimeout(() => {
				this.fire("loaded", {type: "slide", target: this});
	...
	createObjects(objects, callback) {
		...
		for (let i in objects) {
			...
			if (objects[i].constructor === String) {
				...
				parsedObjectsOptions.push(fabric.util.object.clone(objects[i], true))
			}
			else{
...
Object.assign(fabric.Canvas.prototype, CanvasMixin,{
  ...
  _groupSelectedObjects: function (e) {
	...
	// eventListeners: {
	//   "modified loading:before draw:after object:modified canvas:cleared object:added object:removed group:removed canvas:created" : function(){
	//     this.dirty = true;
	//     console.log("dirty");
	//   }
	// },
