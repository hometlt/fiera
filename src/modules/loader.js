...
Object.assign(fabric.StaticCanvas.prototype, {
  createLoader(){
    ...
    this.on("loading:before processing:start", this._showMainLoaderIndicator.bind(this));
    this.on("loaded processing:end", this._hideMainLoaderIndicator.bind(this));
...
Object.assign(fabric.Editor.prototype, {
  ...
  setLoader: function (val) {
    ...
    if(val) {
      ...
      // this.on("loading:before", this._showMainLoaderIndicator);
      // this.on("loaded", this._hideMainLoaderIndicator);
      if(this._loading){
        this._showMainLoaderIndicator()
      }
      if(val.pending){
      ...
      this.on("loading:before", this._showMainLoaderIndicator.bind(this));
      this.on("loaded", this._hideMainLoaderIndicator.bind(this));
      // this.on("slide:loading:before", this._showMainLoaderIndicator.bind(this));
      // this.on("slide:loaded", this._hideMainLoaderIndicator.bind(this));
...
//       this.on("loading:before", function () {
//         this.loaderEl.show();
//         $(this.wrapperEl).addClass("loading");
//       });
